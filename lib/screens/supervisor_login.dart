...
class SupervisorLogin extends StatefulWidget {
  const SupervisorLogin({super.key});

  @override
  _SupervisorLoginState createState() => _SupervisorLoginState();
}
  ...
  Future<void> _login() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('http://10.0.2.2:5000/login'), // Update URL if needed
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success' && data['role'] == 'supervisor') {
          Navigator.pushReplacementNamed(context, '/supervisorDashboard');
        } else {
          _showErrorDialog(data['message'] ?? 'Access denied. Only supervisors can log in.');
        }
